      ...
      pRetry(
        async () => {
          try {
            ...
            throw (pRetry as unknown as { AbortError: new (e: Error) => Error }).AbortError
              ? new (pRetry as unknown as { AbortError: new (e: Error) => Error }).AbortError(
      ...
      const result = await pRetry(
        () => processor(item, index),
        {
          ...
          onFailedAttempt: (error) => {
            if (!isRateLimitError(error)) {
              const A = (pRetry as unknown as { AbortError?: new (e: Error) => Error }).AbortError;
              const wrapped = error instanceof Error ? error : new Error(String(error));
              throw A ? new A(wrapped) : wrapped;
            }
          },
        }
      );
